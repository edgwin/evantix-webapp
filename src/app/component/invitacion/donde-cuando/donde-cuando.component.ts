...
export class DondeCuandoComponent {
  ...
  onClickActividad(id: string) {
    ...
    if (item) { this.tempActividadMap[id] = item.actividad; }
  }

  restoreActividad(item: any, element: HTMLElement) {
    const original = this.tempActividadMap[item.id];
    if (original !== undefined) { element.innerText = original; }
    this.editingActividadId = null;
    element.blur();
  }
  ...
  onClickHora(id: string) {
    ...
    const item = this.data.details.find((d: { id: string }) => d.id === id);
    if (item) { this.tempHoraMap[id] = item.hora; }
  }
  ...
  onClickFecha(id: string) {
    ...
    const item = this.data.details.find((d: { id: string }) => d.id === id);
    if (item) { this.tempFechaMap[id] = item.fecha; }
  }
  ...
  onClickLugar(id: string) {
    ...
    const item = this.data.details.find((d: { id: string }) => d.id === id);
    if (item) { this.tempLugarMap[id] = item.lugar; }
  }
  ...
  onClickDireccion(id: string) {
    ...
    const item = this.data.details.find((d: { id: string }) => d.id === id);
    if (item) { this.tempDireccionMap[id] = item.direccion; }
  }
  ...
  cancelHora() { this.editingHoraId = null; }
  ...
  cancelFecha() { this.editingFechaId = null; }
  ...
  cancelLugar() { this.editingLugarId = null; }
  ...
  cancelDireccion() { this.editingLugarId = null; }
  ...
  uploadBKImage(tableName: string, searchField: string, eventId: string, field: string, file: File) {
    this.invitationService.updateTableFieldImagen(tableName, searchField, eventId, field, file).subscribe({
      next: (res) => { this.data.imagen = res; this.loadingImg = false; },
      error: (err) => {
  ...
  onEscape() {
    if (this.editingActividadId) {
      ...
      if (item && element) { this.restoreActividad(item, element); }
    }
    if (this.editingHoraId) {
      ...
      if (item && element) { this.restoreHora(item, element); }
    }
    if (this.editingFechaId) {
      ...
      if (item && element) { this.restoreFecha(item, element); }
    }
    if (this.editingLugarId) {
      ...
      if (item && element) { this.restoreLugar(item, element); }
    }
    if (this.editingDireccionId) {
      ...
      if (item && element) { this.restoreDireccion(item, element); }
    }
  }
